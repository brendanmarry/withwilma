...
export const POST = async (request: Request) => {
  ...
  try {
    ...
    const input = emailTemplateSchema.parse({ ...(rawBody as Record<string, unknown>), type: "schedule" });
    const template = await generateEmailTemplate(input);
    return NextResponse.json(template);
  } catch (error) {
    logger.error("Failed to generate schedule email", {
      error: serializeError(error),
      request: {
        rawBody,
      },
    });
    return new NextResponse("Failed to generate schedule email", {
      status: 500,
    });
  }
};
